...
- [اپلیکیشن اندروید](#اپلیکیشن-اندروید)
...
اگر سرور مجازی ندارید، در پوشه `android/` یک اپلیکیشن هست که بدون سرور و
بدون هیچ کانفیگی کار می‌کند.

**دانلود مستقیم:** [azad-arm64-v8a-debug.apk](https://github.com/hoseinmovahed88/antitahrim/releases/download/apk-latest/azad-arm64-v8a-debug.apk)

دو روش اتصال دارد و در همان صفحه اول انتخابشان می‌کنید:

- **سرورهای رایگان عمومی** (پیش‌فرض). فهرست‌های عمومی را می‌گیرد، هزاران
  سرور را غربال می‌کند و سریع‌ترین را با هسته Xray بالا می‌آورد. سریع است
  ولی ترافیک از سرور افراد ناشناس رد می‌شود.
- **تونل رایگان Cloudflare**. حساب WARP را روی خود گوشی می‌سازد. به غریبه‌ای
  اعتماد نمی‌کند، ولی روی بعضی شبکه‌های ایران کل پروتکلش مسدود است.

برای پیدا کردن سرورها فقط از DNS‌های داخل ایران استفاده می‌کند (شکن،
الکترو، بگذر، رادار و چند تای دیگر) و اولین بسته دست‌دادن TLS را تکه‌تکه
می‌فرستد تا اسم دامنه از دید سامانه بازرسی پنهان بماند. هر دو از داخل ایران
آزموده شده‌اند.
