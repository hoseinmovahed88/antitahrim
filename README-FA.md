...
به جای سرور شخصی از **Cloudflare WARP** استفاده می‌کند؛ سرویسی که رایگان و
بدون حساب کاربری در دسترس همه است. برنامه خودش روی گوشی یک جفت کلید می‌سازد،
ثبت‌نام می‌کند، و بین ده‌ها ترکیب آی‌پی و پورت می‌گردد تا یکی پیدا کند که
در شبکه شما واقعاً باز باشد.

برای پیدا کردن سرور ثبت‌نام فقط از DNS‌های داخل ایران استفاده می‌کند (شکن،
الکترو، بگذر، رادار و چند تای دیگر) و اولین بسته دست‌دادن TLS را تکه‌تکه
می‌فرستد تا اسم دامنه از دید سامانه بازرسی پنهان بماند.
